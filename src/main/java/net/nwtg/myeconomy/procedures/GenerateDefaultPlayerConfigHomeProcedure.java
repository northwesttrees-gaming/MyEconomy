...
public class GenerateDefaultPlayerConfigHomeProcedure {
					...
					if (localVersion <= 1) {
						{
							try {
								...
								bufferedReader.close();
								mainObject = new Gson().fromJson(jsonstringbuilder.toString(), com.google.gson.JsonObject.class);
								mainObject.addProperty("has_home", (false));
								{
					...
					if (localVersion <= 2) {
						{
							try {
								...
								bufferedReader.close();
								mainObject = new Gson().fromJson(jsonstringbuilder.toString(), com.google.gson.JsonObject.class);
								mainObject.addProperty("home_world", "none");
								{
									Gson mainGSONBuilderVariable = new GsonBuilder().setPrettyPrinting().create();
									try {
										FileWriter fileWriter = new FileWriter(file);
					...
					if (localVersion <= 1) {
						{
							try {
								...
								bufferedReader.close();
								mainObject = new Gson().fromJson(jsonstringbuilder.toString(), com.google.gson.JsonObject.class);
								mainObject.addProperty("home_x", 0);
								mainObject.addProperty("home_y", 0);
								mainObject.addProperty("home_z", 0);
								{
									Gson mainGSONBuilderVariable = new GsonBuilder().setPrettyPrinting().create();
									try {
										FileWriter fileWriter = new FileWriter(file);
