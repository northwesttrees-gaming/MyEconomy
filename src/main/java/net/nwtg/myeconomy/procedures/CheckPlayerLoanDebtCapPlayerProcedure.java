...
public class CheckPlayerLoanDebtCapPlayerProcedure {
	public static void execute(LevelAccessor world, CommandContext<CommandSourceStack> arguments, Entity entity) {
		if (entity == null)
			return;
		double myNumber = 0;
		myNumber = CheckPlayerLoanDebtCapPlayerScriptProcedure.execute(arguments);
		if (myNumber >= 0) {
			if (entity instanceof Player _player && !_player.level.isClientSide())
				_player.displayClientMessage(Component.literal((Component.translatable("msg.myeconomy.check_player_bank_player_name.msg1").getString() + "" + (new Object() {
					public Entity getEntity() {
						...
						+ GetConfigCurrencyIconProcedure.execute(world) + " " + Component.translatable("msg.myeconomy.check_player_loan_debt_cap_player_name.msg1").getString())), (false));
		} else {
			if (entity instanceof Player _player && !_player.level.isClientSide())
