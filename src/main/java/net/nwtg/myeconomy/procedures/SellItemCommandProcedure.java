...
public class SellItemCommandProcedure {
	public static void execute(LevelAccessor world, Entity entity) {
		...
		if (!world.isClientSide()) {
			...
			if (file.exists()) {
				...
				if (sellPrice > 0) {
					itemTotal = ((entity instanceof LivingEntity _livEnt ? _livEnt.getMainHandItem() : ItemStack.EMPTY)).getCount();
					if (entity instanceof Player _player && !_player.level.isClientSide())
						_player.displayClientMessage(
								Component.literal(("\u00A79[NorthGuard]\u00A7f: You sold \u00A7n" + itemTotal + " " + ((entity instanceof LivingEntity _livEnt ? _livEnt.getMainHandItem() : ItemStack.EMPTY).getDisplayName().getString())
										+ "\u00A7r for a total of \u00A7n" + GetConfigCurrencyIconProcedure.execute(world) + " " + sellPrice * itemTotal)),
								(false));
					{
						try {
