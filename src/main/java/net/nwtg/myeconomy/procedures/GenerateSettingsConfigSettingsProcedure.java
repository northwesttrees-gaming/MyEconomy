...
public class GenerateSettingsConfigSettingsProcedure {
					...
					if (localVersion <= 1) {
						{
							try {
								...
								bufferedReader.close();
								mainObject = new Gson().fromJson(jsonstringbuilder.toString(), com.google.gson.JsonObject.class);
								mainObject.addProperty("economy_symbol", "\u00A76\u24BC");
								{
					...
					if (localVersion <= 2) {
						{
							try {
								...
								bufferedReader.close();
								mainObject = new Gson().fromJson(jsonstringbuilder.toString(), com.google.gson.JsonObject.class);
								mainObject.addProperty("anyone_can_use_home_tp", (false));
								mainObject.addProperty("anyone_can_use_spawn_tp", (false));
								{
									Gson mainGSONBuilderVariable = new GsonBuilder().setPrettyPrinting().create();
									try {
										FileWriter fileWriter = new FileWriter(file);
