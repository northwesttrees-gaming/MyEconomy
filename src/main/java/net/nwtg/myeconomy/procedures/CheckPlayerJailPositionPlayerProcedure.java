...
public class CheckPlayerJailPositionPlayerProcedure {
	public static void execute(CommandContext<CommandSourceStack> arguments, Entity entity) {
		...
		myLogic = CheckPlayerJailPositionPlayerIsJailedScriptProcedure.execute(arguments);
		if (myLogic) {
			myPosX = CheckPlayerJailPositionPlayerXScriptProcedure.execute(arguments);
			myPosY = CheckPlayerJailPositionPlayerYScriptProcedure.execute(arguments);
			myPosZ = CheckPlayerJailPositionPlayerZScriptProcedure.execute(arguments);
			if (entity instanceof Player _player && !_player.level.isClientSide())
				...
				}.getEntity()).getDisplayName().getString() + " " + Component.translatable("msg.myeconomy.check_player_jail_pos_player_name.msg1").getString() + new java.text.DecimalFormat("##.##").format(myPosX)
