...
public class CheckPlayerLoanPlayerNameProcedure {
	public static void execute(LevelAccessor world, CommandContext<CommandSourceStack> arguments, Entity entity) {
		if (entity == null)
			return;
		double myNumber = 0;
		myNumber = CheckPlayerLoanPlayerNameScriptProcedure.execute(arguments);
		if (myNumber >= 0) {
			if (entity instanceof Player _player && !_player.level.isClientSide())
				_player.displayClientMessage(Component.literal((Component.translatable("msg.myeconomy.check_player_bank_player_name.msg1").getString() + "" + StringArgumentType.getString(arguments, "playerName") + " "
						...
						+ Component.translatable("msg.myeconomy.check_player_loan_player_name.msg1").getString())), (false));
		} else {
			if (entity instanceof Player _player && !_player.level.isClientSide())
